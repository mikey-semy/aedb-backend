...
class ManualModel(SQLModel):
    ...
    category_id: Mapped[int] = mapped_column(ForeignKey("categories.id", Integer, ondelete="CASCADE", use_alter=True))
    group_id: Mapped[int] = mapped_column(ForeignKey("groups.id", Integer, ondelete="CASCADE", use_alter=True))
    # @declared_attr
    # def category_id(cls):
